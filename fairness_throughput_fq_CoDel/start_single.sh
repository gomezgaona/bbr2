...
while [ $i -lt "${#limit[@]}" ]; do
	...
	echo "BDP = ${BDP[$i]} [bytes]"
	while [ $j -le 10 ]; do
		echo -e "\tRun: $j"
		mkdir "h1_${BDP[$i]}_${j}"
		mkdir "h2_${BDP[$i]}_${j}"
		/home/research/mininet/util/m h1 iperf3 -c 10.0.0.101 -t 3 -J > "h1_${BDP[$i]}_${j}/out.json"&
		/home/research/mininet/util/m h51 iperf3 -c 10.0.0.151 -t 3 -J > "h2_${BDP[$i]}_${j}/out.json"&
		sleep 10
		j=`expr $j + 1`
	done
	i=`expr $i + 1`
done
sudo /home/research/BBR2_new/Figure_9/aggregator_single.sh