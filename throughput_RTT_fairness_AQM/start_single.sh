...
while [ $i -lt "${#limit[@]}" ]; do
	...
	while [ $j -le 10 ]; do
		echo -e "\tRun: $j"
		mkdir "h1_${limit[$i]}_${j}"
		mkdir "h2_${limit[$i]}_${j}"
		/home/research/mininet/util/m h1 iperf3 -c 10.0.0.101 -t 120 -J > "h1_${limit[$i]}_${j}/out.json"&
...
sudo /home/research/BBR2_new/Figure_15/aggregator_single.sh
